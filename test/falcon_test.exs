...
defmodule FalconTest do
  ...
  test "seed" do
    ...
    {:ok, pub, priv} = Falcon.gen_keys_from_seed(seed)
    assert byte_size(pub) == @pub_size, "pubkey no has #{@pub_size} bytes"
    assert byte_size(priv) == @priv_size, "privkey no has #{@priv_size} bytes"
  ...
  test "gen pub from privkey" do
    {:ok, pub, priv} = Falcon.gen_key_pair()
    {:ok, pubk} = Falcon.gen_pub_key(priv)
  ...
  test "sign and verify" do
    msg = "Hello world!"
    {:ok, pub, priv} = Falcon.gen_key_pair()
    {:ok, sig} = Falcon.sign(priv, msg)
    r = Falcon.verify(msg, sig, pub)
  ...
  #             {:ok, s} = Falcon.sign(priv, msg)
  #             r = Falcon.verify(msg, s, pub)
